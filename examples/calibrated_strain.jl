...
const P_input = 125.0              # W (input laser power)

# Cavity parameters
const L_arm = 3995.0               # m (arm cavity length)
const T_ITM = 0.014                # power transmittance of ITM (1.4%)
const T_ETM = 5.0e-6               # power transmittance of ETM (5 ppm)
const Loss_mirror = 37.5e-6        # per-mirror round-trip loss (A+: 37.5 ppm)

# Derived cavity parameters
const F_finesse = π / T_ITM        # finesse ≈ 224
const κ_cavity = T_ITM * c / (2*L_arm)   # Hz (cavity decay rate, HWHM)
const γ_cavity = 2π * κ_cavity     # rad/s (same in angular units)

# Arm cavity loss (per round trip)
const λ_arm = 1 - (1 - Loss_mirror)^2 * (1 - T_ETM)  # ≈ 80 ppm
...
const P_circ = (2*F_finesse/π) * P_input    # ≈ 17.8 kW

# Test mass parameters
const m_mirror = 39.6              # kg (fused silica test mass)
const Ω_mech = 2π * 1.0            # rad/s (pendulum mode ~1 Hz)
const Q_mech = 1e7                 # quality factor (suspension in vacuum)
const Γ_mech = Ω_mech / Q_mech     # rad/s (mechanical damping)

# Optomechanical coupling
const x_zpf = √(ℏ / (2*m_mirror*Ω_mech))      # m (zero-point fluctuation)
const g_0 = (ω_l / L_arm) * x_zpf              # Hz (single-photon coupling)

# Average photon number (simplified)
const n̄_circ = P_circ / (ℏ * ω_l * γ_cavity)  # circulating photons

# Effective coupling (linearized)
const g_eff = 2π * g_0 * √n̄_circ               # rad/s (enhanced coupling)

println("=== LIGO Physical Parameters ===")
println("Arm length L = $(L_arm) m")
println("Finesse F = $(round(F_finesse, digits=1))")
println("Cavity bandwidth κ/(2π) = $(round(κ_cavity, digits=1)) Hz")
println("Circulating power P_circ = $(round(P_circ/1e3, digits=1)) kW")
println("Zero-point fluctuation x_zpf = $(round(x_zpf*1e18, digits=2)) am")
println("Single-photon coupling g_0/(2π) = $(round(g_0, sigdigits=3)) Hz")
println("Photon number n̄ = $(round(n̄_circ, sigdigits=3))")
println("Effective coupling g_eff/(2π) = $(round(g_eff/(2π), sigdigits=3)) Hz")
println()
...
paramdict = Dict([
    ...
    κ => sqrt(γ_cavity),      # cavity decay rate [rad/s]
    Ω => Ω_mech,        # mechanical frequency [rad/s]
    m => m_mirror,      # mirror mass [kg]
    g => g_eff,         # optomechanical coupling [rad/s]
    Γ => sqrt(Γ_mech)         # mechanical damping [rad/s]
])

println("=== Model Parameters ===")
println("Cavity detuning ω = 0 (on resonance)")
println("Cavity length l = $(L_arm) m")
println("κ/(2π) = $(round(γ_cavity/(2π), digits=1)) Hz")
println("Ω/(2π) = $(round(Ω_mech/(2π), digits=2)) Hz")
println("g/(2π) = $(round(g_eff/(2π), sigdigits=3)) Hz")
println("Γ/(2π) = $(round(Γ_mech/(2π), sigdigits=3)) Hz")
println()
